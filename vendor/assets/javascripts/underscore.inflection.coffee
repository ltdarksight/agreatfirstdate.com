...
_s =
  ...
  pluralize: (str, count, includeCount = false) ->
    res = _s.apply_rules str, (if count == 1 then _s.singular_rules else _s.plural_rules), _s.uncountable_words
    if includeCount then "#{count} #{res}" else res
